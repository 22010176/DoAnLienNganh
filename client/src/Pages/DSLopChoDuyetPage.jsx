  ...
  const columns = [
    ...
    },
    {
      title: 'Thao tác', key: 'action', width: 150, align: 'center', render: (_, record) => (
        <Space>
          <Button type="text" icon={<LikeOutlined />} size="small" style={{ color: '#1890ff' }} onClick={() => handleApprove(record)}>
            Duyệt
          </Button>
          <Button type="text" icon={<DislikeOutlined />} size="small" style={{ color: '#ff4d4f' }} onClick={() => handleReject(record)}>
            Không duyệt
          </Button>
        </Space>
      )
